...
Rate limiting is **off by default** in every security mode (api-key, jwt,
and none). When you need it, set:
...
jaiclaw:
  security:
    rate-limit:
      enabled: true
      max-requests-per-window: 60       # requests
      window-seconds: 60                # window
      cleanup-interval-seconds: 300     # purge stale buckets
```

The `RateLimitFilter` is shared by the `api-key` and `jwt` chains; in
`none` mode it does nothing because no authenticated principal is
established to scope the bucket by.

**Why it's opt-in.** Production deployments often sit behind a CDN,
API gateway, or service mesh that already rate-limits. Defaulting the
filter on would either double-count requests or surprise operators whose
upstream policies are strict than the JaiClaw bucket. The trade-off is
intentional: explicit > implicit.

If you want it on for every deployment without per-app config, set
`JAICLAW_SECURITY_RATE_LIMIT_ENABLED=true` in your platform's environment
template (Kubernetes ConfigMap, Helm `values.yaml`, etc.).

### Security headers

All three security modes — `api-key`, `jwt`, and `none` — emit the
following response headers on every request:
...
CSP is **not** currently configured (the variety of channel-adapter
embedded UIs makes a one-size-fits-all CSP painful). Configure your own
via a `SecurityFilterChain` override if your deployment needs it.
