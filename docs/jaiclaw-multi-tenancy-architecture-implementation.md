# JaiClaw Multi-Tenancy Architecture & Implementation Plan
...
Validate tenantId in session token on retrieval.
...
Add a "Multi-Tenancy Conformance Check" section under Key Design Decisions that instructs Claude Code to analyze all code changes for tenant isolation compliance:

- **Persistence layer isolation** — Redis keys, file paths, SQL queries, in-memory map keys, JSON file stores must all be tenant-aware in MULTI mode
- **Async context propagation** — CompletableFuture, virtual threads, Executors must wrap with `TenantContextPropagator`
- **TenantGuard usage** — New components inject `TenantGuard`, not `TenantContextHolder` directly
- **Single-tenant compatibility** — All tenant-aware code must work cleanly in SINGLE mode
- **New MCP tools** — Must pass `TenantContext` through to underlying services

### 10.2 Update CoachIQ CLAUDE.md
**File:** `/Users/tap/dev/workspaces/campaigns/coachiq/CLAUDE.md`

Add conformance guideline: domain models have `tenantId`, repositories filter by tenant, services use `TenantGuard`.
...
Phases 2–10 are independent of each other.
...
|--------|-------|-------------|
| jaiclaw-core | `TenantMode.java` (NEW), `TenantProperties.java` (NEW), `TenantGuard.java` (NEW), `TenantContextPropagator.java` (NEW), `CronJob.java` | Config + utilities + model |
| jaiclaw-spring-boot-starter | `JaiClawCoreAutoConfiguration.java` | Register new beans |
...
| jaiclaw-cron | `CronService.java`, `CronJobExecutor.java`, `JsonFileCronJobStore.java` | Tenant filtering + context |
| jaiclaw-cron-manager | `H2CronJobDefinitionStore.java`, `H2CronExecutionStore.java`, `CronJobManagerService.java`, `CronManagerMcpToolProvider.java`, `CronAgentFactory.java` | Schema + filtering |
...
| jaiclaw-identity | `IdentityLinkService.java`, `IdentityLinkStore.java` | Tenant scoping |
| jaiclaw-subscription | `Subscription.java` (or equivalent), `JsonFileSubscriptionRepository.java` | Model + filtering |
...
| jaiclaw-calendar | `InMemoryCalendarProvider.java` | Key prefix / access validation |
| jaiclaw-browser | `BrowserService.java` | Session key prefix |
| jaiclaw-security | `RateLimitFilter.java`, `ApiKeyAuthenticationFilter.java` | Tenant-aware keys |
| jaiclaw-tools-security | `HandshakeSessionStore.java` | Tenant validation |
| jaiclaw-audit | `AuditLogger.java` | Auto-stamp tenantId |
