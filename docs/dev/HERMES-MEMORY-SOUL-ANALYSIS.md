# Hermes Memory / Soul / Tendencies — Design Analysis

> **Status:** Draft for review.
> **Companion plan:** [`HERMES-MEMORY-SOUL-PLAN.md`](./HERMES-MEMORY-SOUL-PLAN.md)
...
Hermes-agent's three concepts solve a class of problem JaiClaw users have repeatedly
...
| Session-history durability | `core/jaiclaw-memory/src/main/java/io/jaiclaw/memory/SessionTranscriptStore.java` — JSONL transcripts, tenant-aware (`sessions/{tenantId}/{sessionKey}.jsonl` in MULTI) | Hermes uses SQLite+FTS5 here; the JaiClaw equivalent (`SessionTranscriptStore` + `MemorySearchManager`) already exists and the port should **reuse**, not parallel |
...
`HermesMemoryProvider` (extending `MemoryProvider`) and a `MemoryDocument`
...
- **Storage.** JSON-on-disk by default — `${root}/{tenantId}/SOUL.md`
  ...
  `HermesStoreProvider` SPI introduced in Phase 1, but JSON is the default and
...
- **Write API + evolution.** A `soul` agent tool exposes
  ...
  `jaiclaw.hermes.soul.rest.enabled=false` default. Evolution is emergent —
...
- **Tenant-scope variant.** Setting `scope == TENANT` produces an org-wide
  ...
  **operator-only**: REST endpoint `PUT /api/hermes/soul/tenant` (role-guarded
  by `jaiclaw.hermes.soul.tenant.write.roles=ADMIN,OPERATOR`) and the
  ...
  `jaiclaw.hermes.soul.tenant.enabled=false` (default OFF) — independent of
  the pillar-level `jaiclaw.hermes.soul.enabled` toggle.
...
- **Storage.** JSON-on-disk by default — directory layout described in §5
  below. Reuses the same `HermesStoreProvider` SPI as Soul.
- **Prompt path.** A `SessionStartedEvent` listener reads both AGENT and PEER
  documents, captures them into a session attribute `hermes.memory.snapshot`,
...
- **Tenant-scope variant.** Setting `scope == TENANT` produces an org-wide
  ...
  by default** via `PUT /api/hermes/memory/tenant` (role-guarded by
  `jaiclaw.hermes.memory.tenant.write.roles=ADMIN,OPERATOR`). Agents can
  write via the `memory` tool with `scope=TENANT` **only when**
  `jaiclaw.hermes.memory.tenant.agent-write-enabled=true` (default OFF) —
  ...
  Micrometer counter `jaiclaw.hermes.memory.tenant.writes` tagged with
  `agentId` provides an audit trail. Char budget defaults to 4,096
  (configurable via `jaiclaw.hermes.memory.tenant.char-budget`) — larger
  ...
  `jaiclaw.hermes.memory.tenant.enabled=false` (default OFF) — independent
...
- **Tenant-scope variant.** Setting `scope == TENANT` produces an org-wide
  ...
  - Alternative `LlmSummarizingRollupProvider`: one LLM call per rollup;
    opt-in via `jaiclaw.hermes.tendencies.tenant.rollup-provider=llm`.
  - Alternative `ConsensusRollupProvider`: stricter ≥75% threshold; opt-in.
  - Sampling guard: `jaiclaw.hermes.tendencies.tenant.rollup-min-active-users=3`
  ...
  `jaiclaw.hermes.tendencies.tenant.enabled=false` (default OFF) —
...
- **A. Reuse `canonicalUserId`.** Keys are `(tenantId, canonicalUserId)`.
  ...
  `Optional<String> resolveExisting(...)` so the hermes resolver can
...
userKey>` *inside the hermes module*. **`Session.java` is not modified** —
...
|---|---|---|---|
| **Memory (TENANT)** | `${root}/{tenantId}/TENANT.md` (SINGLE: `${root}/TENANT.md`) | PK `(tenant_id)` | `hermes:mem:tenant:{tenantId}` |
| **Memory (AGENT)** | `${root}/{tenantId}/agents/{agentId}/MEMORY.md` (SINGLE: `${root}/agents/{agentId}/MEMORY.md`) | PK `(tenant_id, agent_id)` | `hermes:mem:agent:{tenantId}:{agentId}` |
| **Memory (PEER)** | `${root}/{tenantId}/users/{userKey}/USER.md` | PK `(tenant_id, user_key, agent_id)`; index `(tenant_id, user_key, updated_at DESC)` | `hermes:mem:peer:{tenantId}:{userKey}:{agentId}` |
| **Soul (TENANT)** | `${root}/{tenantId}/TENANT-SOUL.md` (SINGLE: `${root}/TENANT-SOUL.md`) | PK `(tenant_id)` | `hermes:soul:tenant:{tenantId}` |
| **Soul (AGENT)** | `${root}/{tenantId}/agents/{agentId}/SOUL.md` | PK `(tenant_id, agent_id)` | `hermes:soul:{tenantId}:{agentId}` |
| **Tendencies (TENANT)** | `${root}/{tenantId}/TENANT-TENDENCIES.md` (SINGLE: `${root}/TENANT-TENDENCIES.md`) | PK `(tenant_id)` | `hermes:tend:tenant:{tenantId}` |
| **Tendencies (USER)** | `${root}/{tenantId}/users/{userKey}/TENDENCIES.md` | PK `(tenant_id, user_key)`; index `(tenant_id, last_observed DESC)` | `hermes:tend:{tenantId}:{userKey}` |
...
in-flight user chat); Redis CAS uses `WATCH hermes:tend:{tenantId}:{userKey}`
...
(they're marked "Aspirational"). The hermes port depends on all three.
...
keep regardless of the hermes outcome.
...
`HermesUserKeyResolver` is the only internal interface that holds the
...
A Spock spec `HermesStoreIsolationSpec` (mirroring the kanban
`TenantIsolationGuardSpec`) verifies cross-tenant + cross-user reads return
empty. A second spec `HermesMemoryThreeScopeIsolationSpec` verifies the same
...
**Locked-in rule.** Every hermes capability defaults OFF. No autoconfig fires
without explicit operator consent via a `jaiclaw.hermes.{soul,memory,tendencies}
...
|---|---|---|
| `SoulProvider` | `FileSoulProvider` (JSON-on-disk markdown) | `jaiclaw.hermes.soul.enabled=true` |
| `HermesStoreProvider` (returns sub-stores: `SoulStore`, `MemoryStore`, `TendenciesStore`) | `JsonHermesStoreProvider` | one of `jaiclaw.hermes.{soul,memory,tendencies}.enabled=true` + `jaiclaw.hermes.storage.type=json\|h2\|postgres\|redis` (default `json`) |
| `HermesMemoryProvider` (extends `MemoryProvider`) | `BoundedBlobHermesMemoryProvider` | `jaiclaw.hermes.memory.enabled=true` |
| `HermesUserKeyResolver` | `IdentityLinkUserKeyResolver` (delegates to `IdentityResolver`) | always when any hermes concept is enabled; falls back to deterministic hash if `IdentityResolver` absent |
| `TendenciesLearningProvider` | `LocalLlmTendenciesProvider` (uses configured `ChatModel`) | `jaiclaw.hermes.tendencies.enabled=true` |
| Alt: `DeterministicTendenciesLearningProvider` | bundled, opt-in | `jaiclaw.hermes.tendencies.provider=deterministic` |
| Alt: `HonchoRemoteTendenciesProvider` | separate sub-module `extensions/jaiclaw-tendencies-honcho/`, optional Maven dep | `jaiclaw.hermes.tendencies.provider=honcho` + `…honcho.endpoint=…` |
...
| `TenantTendenciesRollupProvider` | `MajorityTraitsRollupProvider` (≥50% active-users threshold, deterministic, no LLM) | `jaiclaw.hermes.tendencies.tenant.enabled=true` |
| Alt: `LlmSummarizingRollupProvider` | bundled, opt-in | `jaiclaw.hermes.tendencies.tenant.rollup-provider=llm` |
| Alt: `ConsensusRollupProvider` | bundled, opt-in (≥75% threshold) | `jaiclaw.hermes.tendencies.tenant.rollup-provider=consensus` |
...
SPIs. `SoulProvider` / `HermesMemoryProvider` / `TendenciesStore`
...
operators must flip both `jaiclaw.hermes.{soul,memory,tendencies}.enabled`
*and* `jaiclaw.hermes.{soul,memory,tendencies}.tenant.enabled` to activate
...
|---|---|---|
| `extensions/jaiclaw-hermes-soul/` | Soul concept + `SoulProvider` SPI default + `soul` agent tool + `BeforePromptBuildEvent` subscriber + MCP provider + REST debug controller + autoconfig | `jaiclaw.hermes.soul.enabled=true` |
| `core/jaiclaw-memory/` (existing module, additions only) | `HermesMemoryProvider` + `MemoryDocument` record + `memory` agent tool + `BoundedBlobMemoryStore` + `MemoryOverflowPolicy` SPI | `jaiclaw.hermes.memory.enabled=true` |
| `extensions/jaiclaw-hermes-tendencies/` | Tendencies concept + `TendenciesStoreProvider` SPI (JSON/JDBC/Redis) + striped dialectic executor + `LocalLlmTendenciesProvider` + `DeterministicTendenciesProvider` + `MessageReceivedEvent` injector + `SessionEndedEvent` cadence trigger + REST + MCP + Actuator | `jaiclaw.hermes.tendencies.enabled=true` |
| `extensions/jaiclaw-tendencies-honcho/` | Optional remote Honcho-compatible provider | `@ConditionalOnClass(HonchoClient.class)` + `jaiclaw.hermes.tendencies.provider=honcho` + endpoint |
| `jaiclaw-starter-hermes-soul/` | Starter pulling soul module | — |
| `jaiclaw-starter-hermes-tendencies/` | Starter pulling tendencies module | — |
| `jaiclaw-starter-hermes/` | Umbrella starter pulling all three | — |
| `jaiclaw-examples/hermes-demo/` | Runnable demo exercising all three pillars with `DeterministicTendenciesLearningProvider` (no LLM key required) | — |
| `.claude/skills/hermes-e2e/` | Out-of-process e2e skill mirroring `kanban-e2e` | — |

**Memory ships inside `core/jaiclaw-memory/`, not a parallel module.** Hermes'
...
`extensions/jaiclaw-hermes-tendencies/`. This keeps `mvn dependency:tree`
readable and the lineage from hermes obvious.

---

## §8 — Per-pillar deep-dives

### §8.1 Hermes-isms — preserve vs adapt

| Hermes choice | Verdict | Rationale |
|---|---|---|
...
| 14 persona overlays as files | **Defer** to Phase 4 as opt-in `jaiclaw.hermes.soul.personas.dir`; shipping the 14 verbatim is itself an open question | Personas are a layer on top of Soul, not core |
...
| Single-tenant single-user assumption | **Extend** | Hermes is desktop-personal; JaiClaw is multi-tenant. TENANT-scope variants of each concept are a JaiClaw extension justified by the multi-tenancy boundary |
...
- **Cost guard.** Per-tenant daily token cap configurable via
  `jaiclaw.hermes.tendencies.cost.daily-token-cap` (default 100k). When the
  cap is hit, the cadence gate trips circuit-breaker open for the rest of the
  day. Actuator endpoint `/actuator/hermes/tendencies` reports passes-per-day,
...
- **Sampling.** `jaiclaw.hermes.tendencies.tenant.rollup-min-active-users=3`
  is the floor; below it the rollup is skipped (logged as INFO).
  `jaiclaw.hermes.tendencies.tenant.active-window=PT168H` (default 7d)
...
- **Output cost gauge.** Reuses the existing
  `jaiclaw.hermes.tendencies.passes.cost.estimated` Micrometer gauge with
...
A Phase 1 spec `HermesPromptCompositionOrderSpec` asserts the ordering;
`HermesScopeFallThroughSpec` covers the three populated-set cases
...
2. **Cache invalidation on Soul/Memory mid-session edits.** A mid-session
   ...
   `jaiclaw.hermes.cache.invalidations` counter; recommend session-end
...
4. **`UserContext` SPI churn deferred.** Option B is the right long-term
   ...
   `HermesUserKeyResolver` insulates call-sites; swapping in a Spring
...
5. **Provider × backend × concept matrix.** Three concepts × N backends × M
   ...
   (`HermesStoreContractSpec`, `TendenciesStoreContractSpec`,
...
7. **Institutional poisoning (tenant Memory).** A bad write to tenant
   ...
   Micrometer counter `jaiclaw.hermes.memory.tenant.writes` with `actor`
...
10. **Cross-agent voice conflict.** Tenant Soul says "concise"; per-agent
    ...
    `jaiclaw.hermes.soul.conflicts` counter; document the layering
...
11. **Prefix-cache invalidation amplified for tenant edits.** A single
    ...
    `jaiclaw.hermes.cache.invalidations` counter with a `scope` label so
...
- **Memory char-budget defaults.** Match hermes (2,200 / 1,375) or rethink
...
- **Cross-agent Soul sharing.** One tenant, multiple agents: does each agent
  ...
  per-agent; per-tenant could be a `jaiclaw.hermes.soul.scope` property.
...
- **Tenant Soul role-guard defaults across deployment styles.** Is
  ...
  `jaiclaw.hermes.soul.tenant.write.roles`. The check is performed
...
- Companion execution plan: [`HERMES-MEMORY-SOUL-PLAN.md`](./HERMES-MEMORY-SOUL-PLAN.md)
...
- Hermes-agent reference: [`github.com/NousResearch/hermes-agent`](https://github.com/NousResearch/hermes-agent)
