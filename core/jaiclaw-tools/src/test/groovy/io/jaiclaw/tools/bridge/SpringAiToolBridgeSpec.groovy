...
class SpringAiToolBridgeSpec extends Specification {
    ...
    def "bridge exposes Spring AI ToolDefinition with correct name and description"() {
        given:
        def jaiclawTool = createTool("test_tool", "A test tool", '{"type":"object","properties":{},"required":[]}')
        def bridge = new SpringAiToolBridge(jaiclawTool, defaultContext)
    ...
    def "call with JSON input deserializes parameters and invokes JaiClaw tool"() {
        given:
        Map<String, Object> capturedParams = null
        def jaiclawTool = createTool("echo", "Echo tool", '{"type":"object","properties":{"msg":{"type":"string"}},"required":["msg"]}') { params, ctx ->
    ...
    def "call with null input passes empty map"() {
        given:
        Map<String, Object> capturedParams = null
        def jaiclawTool = createTool("noop", "No-op", '{"type":"object","properties":{},"required":[]}') { params, ctx ->
            capturedParams = params
    ...
    def "call returns ERROR prefix for ToolResult.Error"() {
        given:
        def jaiclawTool = createTool("fail", "Failing tool", '{"type":"object","properties":{},"required":[]}') { params, ctx ->
    ...
    def "call returns ERROR for malformed JSON input"() {
        given:
        def jaiclawTool = createTool("strict", "Strict", '{"type":"object","properties":{},"required":[]}') { params, ctx ->
    ...
    def "call propagates default context to JaiClaw tool"() {
        given:
        ToolContext capturedContext = null
        def jaiclawTool = createTool("ctx", "Context tool", '{"type":"object","properties":{},"required":[]}') { params, ctx ->
            capturedContext = ctx
    ...
    def "unwrap returns original JaiClaw tool"() {
        given:
        def jaiclawTool = createTool("orig", "Original", '{"type":"object","properties":{},"required":[]}')
        def bridge = new SpringAiToolBridge(jaiclawTool, defaultContext)

        expect:
        bridge.unwrap().is(jaiclawTool)
    }
