...
- **Compliance substrate** — one property (`jaiclaw.compliance.profile={gdpr|hipaa|both|soc2|fedramp-moderate|cmmc-l2|fips}`) turns on retention enforcement, LLM-call auditing with recipient tracking, BAA-eligible provider warnings, HTTPS startup guard, PHI redaction, and a REST surface at `/api/gdpr/*` for Art. 15/17/20 subject requests. 12 `@Stable` SPIs covering data-subject erasure + export, consent, prompt redaction, field encryption, chain-of-hashes audit, privacy notice, RoPA generation. `soc2` adds a tamper-evident audit chain, encryption at rest, and a `MINIMAL` default tool profile. Zero code loads when profile=none. ([compliance guide](docs/user/COMPLIANCE.md), [regulation deep-dives](docs/compliance/README.md))
