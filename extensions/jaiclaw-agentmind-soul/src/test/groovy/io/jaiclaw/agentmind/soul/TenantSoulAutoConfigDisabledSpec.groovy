...
class TenantSoulAutoConfigDisabledSpec extends Specification {
    ...
    def "pillar=true + tenant.enabled=false explicitly: still no tenant beans"() {
        ...
        runner.withPropertyValues(
                ...
                "jaiclaw.agentmind.soul.tenant.enabled=false"
        ).run { ctx ->
            assert ctx.getBeanNamesForType(TenantSoulController).length == 0
            assert ctx.getBeanNamesForType(TenantSoulMcpToolProvider).length == 0
        }
    ...
    def "pillar=false + tenant.enabled=true: no beans at all (pillar gates everything)"() {
        expect:
        runner.withPropertyValues(
                "jaiclaw.agentmind.soul.enabled=false",
                "jaiclaw.agentmind.soul.tenant.enabled=true"
        ).run { ctx ->
            assert !ctx.containsBean("agentmindSoulModuleMarker")
            assert ctx.getBeanNamesForType(TenantSoulController).length == 0
    ...
    def "rest.enabled=true wires the SoulDebugController only when pillar is on"() {
        ...
        runner.withPropertyValues(
                "jaiclaw.agentmind.soul.enabled=true",
                "jaiclaw.agentmind.soul.rest.enabled=true"
        ).run { ctx ->
            assert ctx.getBeanNamesForType(io.jaiclaw.agentmind.soul.web.SoulDebugController).length == 1
        }
    }
    ...
    def "rest.enabled=false is the default — no debug controller wired"() {
        expect:
        runner.withPropertyValues("jaiclaw.agentmind.soul.enabled=true").run { ctx ->
            assert ctx.getBeanNamesForType(io.jaiclaw.agentmind.soul.web.SoulDebugController).length == 0
        }
    }
}
