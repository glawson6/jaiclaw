...
/**
 * Configuration surface for {@code jaiclaw-hermes-soul}.
 *
 ...
 * </ul>
 */
@ConfigurationProperties(prefix = "jaiclaw.hermes.soul")
public record HermesSoulProperties(
        boolean enabled,
        String rootDir,
        Rest rest,
        Tenant tenant
    ...
    @ConstructorBinding
    public HermesSoulProperties {
        if (rootDir == null || rootDir.isBlank()) {
            rootDir = System.getProperty("user.home") + "/.jaiclaw/hermes/soul";
        }
    ...
    /**
     * Debug read endpoint at {@code GET /api/hermes/soul}. Defaults OFF;
     * intended for ops use only, not for runtime consumers.
     */
    public record Rest(boolean enabled) {}

    /**
     * Tenant-scope sub-configuration. Both {@link HermesSoulProperties#enabled()}
