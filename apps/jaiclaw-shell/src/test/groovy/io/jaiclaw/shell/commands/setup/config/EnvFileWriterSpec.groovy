...
class EnvFileWriterSpec extends Specification {
    ...
    def "includes security mode in env file"() {
        ...
        then:
        env.contains("export JAICLAW_SECURITY_MODE=api-key")
        !env.contains("JAICLAW_API_KEY=")
    }
    ...
    def "includes custom API key in env file when set"() {
        ...
        then:
        env.contains("export JAICLAW_SECURITY_MODE=api-key")
        env.contains("export JAICLAW_API_KEY=my-custom-key")
    }
    ...
    def "includes security mode none in env file"() {
        ...
        then:
        env.contains("export JAICLAW_SECURITY_MODE=none")
        !env.contains("JAICLAW_API_KEY=")
    }
