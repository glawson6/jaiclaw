...
#   JAICLAW_ENV_FILE        Path to .env file for additional config
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
MODULE_ARTIFACT="jaiclaw-example-system-monitor"
...
load_env() {
    local env_file="${JAICLAW_ENV_FILE:-$PROJECT_ROOT/docker-compose/.env}"
...
validate_config() {
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        ...
        echo ""
        echo "Set them in your environment or in \$JAICLAW_ENV_FILE:"
...
print_help() {
    ...
    echo ""
    echo "Zero-prerequisite launcher for the JaiClaw System Monitor example."
    ...
    echo "  JAICLAW_ENV_FILE        Path to .env file for additional config"
    echo ""
    echo "Examples:"
