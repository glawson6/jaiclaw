...
How we built a server monitoring agent that uses an AI backbone to collect system metrics, analyze them, and deliver daily health reports via Telegram — using JaiClaw's Composable App Assembly pattern.
...
The system monitor is a JaiClaw example application that demonstrates the **Composable App Assembly** pattern: composing JaiClaw starters to build a purpose-built agent in minimal code.
...
The entire application has 2 Java files and 1 Markdown skill file. Everything else — the agent runtime, cron scheduling, session management, tool bridging, Telegram integration — comes from JaiClaw's starters.

## The Composable App Assembly Pattern

Instead of writing everything from scratch, the system monitor composes three JaiClaw starters:
...
The most critical design decision: how to let an AI execute shell commands safely. JaiClaw provides `WhitelistedCommandTool`, which checks every command against a prefix allowlist:
...
The agent needs to deliver reports to a specific Telegram chat. JaiClaw's `jaiclaw-channel-telegram` module provides a reusable `SendTelegramTool` that any app can wire as a bean:
...
Skills are system prompt fragments — they tell the AI *how* to use tools, what workflow to follow, and how to format output. JaiClaw's `SkillLoader` reads them from the classpath, parses the frontmatter, and injects them into the agent's system prompt. You can edit skills without recompilation.
...
Everything else — the agent runtime, cron scheduling, Telegram integration, tool bridging, session management, H2 persistence — comes from the three JaiClaw starters.

## Getting Started

1. Clone the JaiClaw repository
2. Build:
   ```bash
...
- **JaiClaw 0.1.0** — AI agent framework (gateway, cron manager, channels, tools, skills)
...
*This example is part of [JaiClaw](https://github.com/taptech/jaiclaw), an open-source Java framework for building embeddable AI agents with multi-channel messaging.*
